...
import { PendingStartup } from '../../types/admin';
...
export function AdminPortal({ user, navigate }: AdminPortalProps) {
  ...
  // Fetch pending startups from API
  const fetchPendingStartups = async () => {
    try {
      ...
      toast.error('Failed to fetch pending startups');
    }
  };
  
  // Add fetch function for counts
  const fetchDashboardCounts = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('http://localhost:5139/api/Admin/counts', {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Accept': '*/*'
        }
  ...
  const handleApprove = async (id: number) => {
    try {
      ...
      fetchPendingStartups(); // Refresh the list
    } catch (error) {
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <h3 className="text-2xl font-bold mt-1">{dashboardCounts.totalClients.toLocaleString()}</h3>
                      </div>
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <h3 className="text-2xl font-bold mt-1">{dashboardCounts.totalStartups.toLocaleString()}</h3>
                      </div>
                              ...
                              <Avatar className="h-10 w-10">
                                {startup.logoPath ? (
                                  ...
                                  <AvatarFallback className="bg-muted text-primary">
                                    {startup.displayName.substring(0, 2).toUpperCase()}
                                  </AvatarFallback>
                                )}
                              </Avatar>
                              <div>
                                <p className="text-sm font-medium">{startup.displayName}</p>
                  ...
                  <Card>
                    ...
                    <CardContent className="p-0">
                      <div className="flex items-center justify-center p-8 text-muted-foreground">
                        ...
                        <p>Support structure management functionality is under development</p>
                      </div>
                    </CardContent>
                  </Card>
                </div>
              ...
              <Card>
                ...
                <CardContent>
                  <StartupApproval 
                    ...
                    onView={(id: number) => console.log(`View startup: ${id}`)}
                  />
                </CardContent>
              </Card>
            </TabsContent>
            ...
            <TabsContent value="support" className="space-y-6">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Coming soon - Support structure management will be implemented in a future update
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="flex items-center justify-center p-8 text-muted-foreground">
