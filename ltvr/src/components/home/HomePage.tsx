...
export function HomePage() {
  ...
  const nextStartupSlide = () => {
    setCurrentStartupSlide((prev) => (prev === startups.length - 3 ? 0 : prev + 1));
  };

  const prevStartupSlide = () => {
    setCurrentStartupSlide((prev) => (prev === 0 ? startups.length - 3 : prev - 1));
  };

  // Functions to handle slides for support structures
  const nextSupportSlide = () => {
    setCurrentSupportSlide((prev) => (prev === incubators.length - 3 ? 0 : prev + 1));
  };

  const prevSupportSlide = () => {
    setCurrentSupportSlide((prev) => (prev === 0 ? incubators.length - 3 : prev - 1));
  };
  ...
  useEffect(() => {
    const fetchIncubators = async () => {
      try {
        ...
        setIncubatorsError(err instanceof Error ? err.message : 'Failed to load incubators');
      } finally {
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          <Tabs defaultValue="startups" value={activeTab} onValueChange={setActiveTab} className="w-full">
                ...
                <>
                  ...
                  <div className="overflow-hidden">
                    <div 
                      ...
                      style={{ transform: `translateX(-${currentStartupSlide * (100 / displayStartups.length) * 3}%)` }}
                    >
            ...
            <TabsContent value="support" className="outline-none">
              {isIncubatorsLoading ? (
                <div className="text-center py-12">Loading support structures...</div>
              ) : incubatorsError ? (
                <div className="text-center text-red-500 py-12">{incubatorsError}</div>
              ) : (
                <>
                  <div className="flex items-center justify-between mb-8">
                    <div className="flex items-center">
                  ...
                  <div className="overflow-hidden">
                    <div 
                      className="flex transition-transform duration-500 ease-in-out"
                      style={{ transform: `translateX(-${currentSupportSlide * (100 / incubators.length) * 3}%)` }}
                    >
                      {incubators.map((item) => (
                        <div 
                        ...
                        >
                          ...
                          >
                            ...
                            <div className="p-6">
                              <h3 className="text-xl font-semibold mb-2">{item.incubatorInfo.displayName}</h3>
                              <p className="text-gray-400 text-sm mb-4">
                                {item.incubatorInfo.country || 'International'} based {item.incubatorInfo.role} supporting innovative startups.
                              </p>
                              ...
                              <div className="grid grid-cols-2 gap-4 mb-4">
                                <div className="bg-[#0c131d] rounded-lg p-3 border border-gray-800">
                                  ...
                                  <p className="text-xl font-bold text-white">{item.startupsCount}</p>
                                </div>
                                <div className="bg-[#0c131d] rounded-lg p-3 border border-gray-800">
                              ...
                              <div className="flex flex-wrap gap-2 mb-4">
                                {[item.incubatorInfo.role, item.incubatorInfo.country || 'Global', 'Innovation'].map((tag) => (
                                  <span 
                                    key={tag} 
